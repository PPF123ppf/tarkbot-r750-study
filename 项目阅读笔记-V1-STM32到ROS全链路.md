...
注意 **FreeRTOS 默认配置下数字越大优先级越高**——所以最关键的 `Robot_Task` 反而设成中等优先级（3），比 OLED/灯/按键都低。这样设计的逻辑是：`Robot_Task` 每周期跑完只占 1–5ms，**剩下的 15ms 都在 Blocked 状态**（被 `vTaskDelayUntil` 阻塞等下个 20ms 节拍），CPU 自然轮给高优先级的"短任务"（OLED 刷新、按键扫描、灯效更新）。

`vTaskDelayUntil` 的关键是**精确节拍**：唤醒时间 = `(上次唤醒时刻) + 20ms`，跟本次干活耗时无关。即使 Robot 跑到一半被 Key/Disp 抢占几次，只要 **总占用（干活 + 被抢）< 20ms**，下个 50Hz 节拍依然准时到达。注意它不能阻止抢占——抢占规则只看优先级，跟"频率"或"用什么 delay 函数"无关。

把 Robot 设中等优先级而不是最高，反而有个好处：**万一 Robot 卡死了**（比如 IMU I²C 卡 bus、PID 算出 NaN 死循环），**高优先级的短任务还能跑、用 OLED/蜂鸣给用户报警**。如果把 Robot 设成最高优先级，它一旦卡死整机就黑屏不响，用户根本不知道出问题了。这是嵌入式工程常用的"看门狗任务模式"——让"用户感知/状态汇报"任务高于"工作主力"任务。
